    ...
    return (
      <div className="rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 p-3 shadow-xl text-xs">
        <div className="flex items-center justify-between pb-1.5 border-b border-slate-100 dark:border-slate-800 gap-4">
          <span className="font-bold text-slate-900 dark:text-white text-sm">{data.time}</span>
        ...
        <div className="mt-2 space-y-1">
          <div className="flex justify-between text-slate-600 dark:text-slate-300">
            ...
            <span className="font-bold text-emerald-600 dark:text-teal-400 font-mono text-sm">{data.aqi}</span>
          </div>
          <div className="flex justify-between text-slate-500 dark:text-slate-400">
            <span>PM2.5 / PM10:</span>
            <span className="font-mono text-slate-700 dark:text-slate-200">{data.pm25} / {data.pm10} µg/m³</span>
          </div>
          <div className="flex justify-between text-slate-500 dark:text-slate-400">
            <span>Confidence Range:</span>
            <span className="font-mono text-slate-600 dark:text-slate-300">{data.confidenceLower} - {data.confidenceUpper}</span>
          </div>
        </div>
      </div>
    );
...
export const Forecast24HourChart: React.FC<Forecast24HourChartProps> = ({
  ...
  const validData = data && data.length > 0 ? data : HOURLY_FORECAST_24H;
  const peakForecast = [...validData].sort((a, b) => b.aqi - a.aqi)[0] || validData[0];
      ...
      {peakForecast && (
        <div className="mt-4 p-3 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 text-xs">
          <div className="flex items-center space-x-2 text-slate-700 dark:text-slate-300">
            ...
            <span>
              <strong>Model Projection:</strong> Peak level expected around <strong>{peakForecast.time}</strong> (AQI ~ {peakForecast.aqi} - {peakForecast.category}).
            </span>
          </div>
      ...
      <div className="mt-5 h-64 w-full">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={validData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
            ...
            <ReferenceLine y={100} stroke="#f59e0b" strokeDasharray="3 3" label={{ value: 'Moderate Limit (100)', fill: '#f59e0b', fontSize: 10 }} />
            <ReferenceLine y={200} stroke="#ef4444" strokeDasharray="3 3" label={{ value: 'Poor Limit (200)', fill: '#ef4444', fontSize: 10 }} />
            <Area
              type="monotone"
              dataKey={selectedMetric === 'aqi' ? 'aqi' : selectedMetric === 'pm25' ? 'pm25' : 'pm10'}
              stroke="#10b981"
              strokeWidth={2}
              fillOpacity={1}
