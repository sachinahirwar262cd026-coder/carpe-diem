...
export const ComplaintForm: React.FC<ComplaintFormProps> = ({ onSuccess }) => {
  ...
  const [latitude, setLatitude] = useState<string>(selectedCity.center[0].toFixed(6));
  const [longitude, setLongitude] = useState<string>(selectedCity.center[1].toFixed(6));
  const [locationName, setLocationName] = useState<string>(`${selectedCity.name} Central Zone`);
  ...
  const handleUseCurrentLocation = async () => {
    ...
    try {
      ...
      setLocationName(geoInfo.displayName || `${pos.lat.toFixed(4)}, ${pos.lng.toFixed(4)}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formData.append('detectedIntensity', detectedIntensity || '84.2 dB(A) Exceeded CPCB Standard');
      ...
      // 2. Dispatch to Backend API endpoint (/api/complaints)
      try {
        const token = localStorage.getItem('token') || localStorage.getItem('sih_auth_token');
        ...
        console.warn('Backend API submission note (fallback to local state store):', apiErr);
      }

      // 3. Update client context store
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="p-4 rounded-xl bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800/60 flex items-center space-x-3">
        ...
        <div>
          ...
          <p className="text-xs text-blue-700 dark:text-blue-300">
            Submit 10-second audio recording with GPS coordinates. Form data is dispatched with converted Mel-Spectrogram for AI validation.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div>
          <label className="block text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 mb-1.5">
            Noise Grievance Summary Title <span className="text-rose-500">*</span>
          </label>
          <input
            type="text"
            required
      ...
      >
        {isSubmitting ? (
          <span>Transmitting Form-Data &amp; Spectrogram to Enforcement Queue...</span>
        ) : (
          <>
